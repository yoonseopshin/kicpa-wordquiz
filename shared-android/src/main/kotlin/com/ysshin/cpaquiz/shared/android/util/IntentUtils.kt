...
inline fun <reified T : Serializable?> Bundle.serializable(key: String, clazz: Class<T>): T? = when {
    Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU -> getSerializable(key, clazz)
    else -> getSerializable(key) as? T
}

inline fun <reified T : Parcelable?> Bundle.parcelable(key: String, clazz: Class<T>): T? = when {
    Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU -> getParcelable(key, clazz)
    else -> getParcelable(key) as? T
}

inline fun <reified T : Parcelable?> Bundle.parcelableArrayList(key: String, clazz: Class<T>): ArrayList<T>? =
    when {
